...
class ExampleUnitTest {
    ...
    fun parse_header() {
        ...
        assertEquals(listOf(1,2,3,4,5,6), actualLevels) // проверка уровней header-ов.
    }

    @Test
    fun parse_quote() { // цитаты
    ...
    @Test
    fun parse_all() { // добавить multiline +  orderedlist
    ...
    private fun Element.spread(): List<Element> {
        ...
        elements.add(this) // Добавляем родительский элемент
        elements.addAll(this.elements.spread()) // Добавляем дочернии элементы
        return elements
    }

    private fun List<Element>.spread(): List<Element> {// Развернуть дочерние элементы в последовательность
        val elements = mutableListOf<Element>()
    ...
    private fun printResults(list:List<String>){ // вывод на печать коллекции строк
    ...
    private fun printElements(list:List<Element>){ // вывод на печать элементы (data class-ы)
        val iterator = list.iterator()
        while (iterator.hasNext()){
